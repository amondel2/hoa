...
<html>
	...
	<body>
		...
		<div id="edit-messRole" class="content scaffold-edit" role="main">
			<h1><g:message code="default.edit.label" args="[entityName]" /></h1>
			<g:if test="${flash.message}">
			<div class="message" role="status">${flash.message}</div>
			</g:if>
			<g:hasErrors bean="${messRoleInstance}">
			<ul class="errors" role="alert">
				<g:eachError bean="${messRoleInstance}" var="error">
				<li <g:if test="${error in org.springframework.validation.FieldError}">data-field-id="${error.field}"</g:if>><g:message error="${error}"/></li>
				</g:eachError>
			</ul>
			</g:hasErrors>
			<g:form url="[resource:messRoleInstance, action:'update']" method="PUT" >
				<g:hiddenField name="version" value="${messRoleInstance?.version}" />
				<fieldset class="form">
